...
public class NetworkOperations {
  ...
  public static final Function<TransferState, Result<TransferState, IrrecoverableError>> receiveValidAck = (state) -> {
    ...
    final Recursive<BiFunction<Integer, Integer, Result<TransferState, IrrecoverableError>>> receiveAck =
        new Recursive<>();
  ...
  public static final Function<TransferState, Result<TransferState, IrrecoverableError>> receiveValidDataPacket = (state) -> {
    final long tsStart = currentTime.get();
    Supplier<Integer> calculateNewTimeout = () -> {
      Long to = Configuration.TIMEOUT_TIME - (currentTime.get() - tsStart);
      return to.intValue();
