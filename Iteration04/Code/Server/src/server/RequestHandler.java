...
class RequestHandler implements Runnable {
  ...
  private void sendFileToClient(ReadRequest request) {
    ...
    try {
      ...
      // Filler packet which contains the IP and Port of the client.
      // Could overload sendErrorPacket or if you know a better way to do this, feel free to implement it.
      byte[] buffer = new byte[512];
      DatagramPacket responseDatagram = new DatagramPacket(buffer, buffer.length, request.getRemoteHost(), request.getRemotePort());
      
      String errMsg = "The file \"" + request.getFilename() + "\" could not be found on the server.";
      sendErrorPacket(errMsg, ErrorCode.FILE_NOT_FOUND, responseDatagram);
      ...
      do {
        ...
        while (ack == null) {
          ...
          if (ack.getBlockNumber() < blockNumber) {
            ...
            log(errMsg);
            sendErrorPacket(errMsg, ErrorCode.ILLEGAL_TFTP_OPERATION, responseDatagram);
            errorOccured = true;
            break;
          }
        }
  ...
  private void receiveFileFromClient(WriteRequest request) {
    ...
    try {
      ...
      // TODO Throw this exception where appropriate
      ...
      byte[] buffer = new byte[512];
      DatagramPacket responseDatagram = new DatagramPacket(buffer, buffer.length, request.getRemoteHost(), request.getRemotePort());
      String errMsg = "The file \"" + request.getFilename() + "\" already exists on the server.";
      sendErrorPacket(errMsg, ErrorCode.FILE_ALREADY_EXISTS, responseDatagram);
      ...
      DatagramPacket responseDatagram = new DatagramPacket(buffer, buffer.length, request.getRemoteHost(), request.getRemotePort());
      String errMsg = "The file \"" + request.getFilename() + "\" could not be found on the server.";
      sendErrorPacket(errMsg, ErrorCode.DISK_FULL_OR_ALLOCATION_EXCEEDED, responseDatagram);
    ...
    while (!errorOccured) {
      ...
      try {
        ...
        // TODO create a scenario to test this 
        // Filler packet which contains the IP and Port of the client.
        // Could overload sendErrorPacket or if you know a better way to do this, feel free to implement it.
        byte[] buffer = new byte[512];
        DatagramPacket responseDatagram = new DatagramPacket(buffer, buffer.length, request.getRemoteHost(), request.getRemotePort());
        String errMsg = "Not enough memory on the disk to write file.";
        sendErrorPacket(errMsg, ErrorCode.DISK_FULL_OR_ALLOCATION_EXCEEDED, responseDatagram);
  ...
  private void handleParseError(String message, DatagramPacket datagram) {
    ...
    try {
      ...
      sendErrorPacket(message, ErrorCode.ILLEGAL_TFTP_OPERATION, datagram);
      return;
    }
  ...
  private void sendErrorPacket(String message, ErrorCode errorCode, DatagramPacket requestPacket) {
