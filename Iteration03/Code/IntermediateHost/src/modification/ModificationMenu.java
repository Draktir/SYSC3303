...
public class ModificationMenu {
	...
	public PacketModifier show() {
		...
		while (menuSelection < 0 || menuSelection > 4) {
			...
			System.out.println("  [ 5 ] Delay or Duplicate a packet (timeout)");
	...
	private void delayMenu() {
		int delaytype;
		do {
			System.out.println("what kind of packet do you wana delay ?");
			System.out.println("  [ 1 ] Data Packets");
			System.out.println("  [ 2 ] ACK Packets");

			delaytype = scan.nextInt();
		} while (delaytype != 0);

		int command;
		do {
			System.out.println("Do you wana Delay or Duplicate?");
			...
			case 1:
				int sleeptime;
				System.out.println("how long is the delay?");
		...
		int packnumber;
		do{
			System.out.println("which Packet do you want ?");
			packnumber = scan.nextInt();
		}while (packnumber != 0);
	}

	private byte[] getByteUserInput() {
		System.out.print("Enter bytes separated by spaces:");
		scan.nextLine(); // don't ask me why we need this, Java's Scanner is a
							// pain in the ...
