...
Testing Error Code 5 (Invalid TID)
  ...
  it receives an error from the receiving host it will then send the same packet again, but on 
  the original socket with the correct port. This shows that the receiving host will keep running
  if an Error Code 5 occurs and the transfer can continue.
